...
//Dark  features
bool fFloatingCity = false;
string strFloatingCityPrivKey = "";
string strFloatingCityAddr = "";
bool fLiteMode = false;
bool fEnableInstantX = true;
int nInstantXDepth = 10;
int nMNengineRounds = 2;
int nAnonymizeZalemCoinAmount = 1000;
int nLiquidityProvider = 0;
/** Spork enforcement enabled time */
int64_t enforceFloatingcityPaymentsTime = 4085657524;
int nFloatingcityMinProtocol = 0;
...
#else
    const char* pszModule = "Zalem-Coin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZLM
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZLM
    // Mac: ~/Library/Application Support/ZLM
    // Unix: ~/.ZLM
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZLM";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Zalem-Coin.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;
    return pathConfigFile;
}

boost::filesystem::path GetFloatingcityConfigFile()
{
    boost::filesystem::path pathConfigFile(GetArg("-mnconf", "floatingcity.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "Zalem-Coin.conf";
               ...
               fprintf(ConfFile, "port=30099\n");
               fprintf(ConfFile, "rpcport=30097\n");
               fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
               fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
               fprintf(ConfFile, "addnode=134.209.57.3:30099\n");
               fprintf(ConfFile, "addnode=138.197.168.172:30099\n");
               fprintf(ConfFile, "addnode=206.189.73.71:30099\n");
               fprintf(ConfFile, "addnode=138.197.161.183:30099\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Zalem-Coind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Zalem-Coin will not work properly.");
