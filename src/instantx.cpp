...
//step 2.) Top 10 floatingcities, open connect to top 1 floatingcity. Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 floatingcity, waits for 10 messages. Upon success, sends "txlock'

void ProcessMessageInstantX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if(fLiteMode) return; //disable all fcengine/floatingcity related functionality
    ...
    else if (strCommand == "txlvote") //InstantX Lock Consensus Votes
    {
        ...
        if(ProcessConsensusVote(pfrom, ctx)){
            //Spam/Dos protection
            /*
                Floatingcities will sometimes propagate votes before the transaction is known to the client.
            ...
            */
            if(!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)){
                if(!mapUnknownVotes.count(ctx.vinFloatingcity.prevout.hash)){
                    mapUnknownVotes[ctx.vinFloatingcity.prevout.hash] = GetTime()+(60*10);
                }

                if(mapUnknownVotes[ctx.vinFloatingcity.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinFloatingcity.prevout.hash] - GetAverageVoteTime() > 60*10){
                        LogPrintf("ProcessMessageInstantX::txlreq - floatingcity is spamming transaction votes: %s %s\n",
                            ctx.vinFloatingcity.ToString().c_str(),
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which floatingcities
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetFloatingcityRank(activeFloatingcity.vin, nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

    if(n == -1)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Unknown Floatingcity\n");
        return;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Floatingcity not in the top %d (%d)\n", INSTANTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinFloatingcity = activeFloatingcity.vin;
...
{
    int n = mnodeman.GetFloatingcityRank(ctx.vinFloatingcity, ctx.nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

    CFloatingcity* pmn = mnodeman.Find(ctx.vinFloatingcity);
    if(pmn != NULL)
    {
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Floatingcity ADDR %s %d\n", pmn->addr.ToString().c_str(), n);
    }

    if(n == -1)
    {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Unknown Floatingcity\n");
        mnodeman.AskForMN(pnode, ctx.vinFloatingcity);
        return false;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Floatingcity not in the top %d (%d) - %s\n", INSTANTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if(!ctx.SignatureValid()) {
        LogPrintf("InstantX::ProcessConsensusVote - Signature invalid\n");
        //don't ban, it could just be a non-synced floatingcity
        mnodeman.AskForMN(pnode, ctx.vinFloatingcity);
        return false;
    }
...
{
    return vinFloatingcity.prevout.hash + vinFloatingcity.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    CFloatingcity* pmn = mnodeman.Find(vinFloatingcity);

    if(pmn == NULL)
    {
        LogPrintf("InstantX::CConsensusVote::SignatureValid() - Unknown Floatingcity\n");
        return false;
    }

    if(!mnEngineSigner.VerifyMessage(pmn->pubkey2, vchFloatingCitySignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strFloatingCityPrivKey.c_str());

    if(!mnEngineSigner.SetKey(strFloatingCityPrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid floatingcityprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if(!mnEngineSigner.SignMessage(strMessage, errorMessage, vchFloatingCitySignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if(!mnEngineSigner.VerifyMessage(pubkey2, vchFloatingCitySignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    ...
    {
        int n = mnodeman.GetFloatingcityRank(vote.vinFloatingcity, vote.nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

        if(n == -1)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Floatingcity\n");
            return false;
        }

        if(n > INSTANTX_SIGNATURES_TOTAL)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Floatingcity not in the top %s\n", INSTANTX_SIGNATURES_TOTAL);
            return false;
        }
