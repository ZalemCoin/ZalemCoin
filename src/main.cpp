...
const string strMessageMagic = "Zalem-Coin Signed Message:\n";
                ...
                if(pindex != NULL){
                    if(pindex->GetBlockHash() == hashPrevBlock){
                        ...
                        if (Params().NetworkID() == CChainParams::MAIN)
                            devopaddress = CBitcoinAddress("ZXUiExq15ATbkeEBB2FUXFqpJQGKk2iGAd");
                        ...
                        {
                            //spork
                            if(pindexBest->GetBlockTime() > 1554091740) { // ON (Sunday, March 31, 2019 9:09:00 PM GMT-07:00)
...
// ----------- instantX transaction scanning -----------
    ...
    bool FloatingcityPayments = false;
    bool fIsInitialDownload = IsInitialBlockDownload();

    int64_t cTime = nTime;
    int64_t mTime = START_FLOATINGCITY_PAYMENTS;
    if(cTime > mTime) FloatingcityPayments = true;
    if (!fIsInitialDownload)
    {
        if(FloatingcityPayments)
        {
            LOCK2(cs_main, mempool.cs);
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    // If we don't already have its previous block, skip floatingcity payment step
                    CAmount floatingcityPaymentAmount;
                    for (int i = vtx[1].vout.size(); i--> 0; ) {
                        floatingcityPaymentAmount = vtx[1].vout[i].nValue;
                        break;
                    }
                    ...
                    CScript payee;
                    CTxIn vin;
                    if(!floatingcityPayments.GetBlockPayee(pindexBest->nHeight+1, payee, vin) || payee == CScript()){
                        foundPayee = true; //doesn't require a specific payee
                        foundPaymentAmount = true;
                        foundPaymentAndPayee = true;
                        if(fDebug) { LogPrintf("CheckBlock() : Using non-specific floatingcity payments %d\n", pindexBest->nHeight+1); }
                    }

                    for (unsigned int i = 0; i < vtx[1].vout.size(); i++) {
                        if(vtx[1].vout[i].nValue == floatingcityPaymentAmount )
                            foundPaymentAmount = true;
                        if(vtx[1].vout[i].scriptPubKey == payee )
                            foundPayee = true;
                        if(vtx[1].vout[i].nValue == floatingcityPaymentAmount && vtx[1].vout[i].scriptPubKey == payee)
                            foundPaymentAndPayee = true;
                    }
                    ...
                    if(!foundPaymentAndPayee) {
                        if(fDebug) { LogPrintf("CheckBlock() : Couldn't find floatingcity payment(%d|%d) or payee(%d|%s) nHeight %d. \n", foundPaymentAmount, floatingcityPaymentAmount, foundPayee, address2.ToString().c_str(), pindexBest->nHeight+1); }
                        return DoS(100, error("CheckBlock() : Couldn't find floatingcity payment or payee"));
                    } else {
                        LogPrintf("CheckBlock() : Found payment(%d|%d) or payee(%d|%s) nHeight %d. \n", foundPaymentAmount, floatingcityPaymentAmount, foundPayee, address2.ToString().c_str(), pindexBest->nHeight+1);
                    }
                } else {
                    if(fDebug) { LogPrintf("CheckBlock() : Skipping floatingcity payment check - nHeight %d Hash %s\n", pindexBest->nHeight+1, GetHash().ToString().c_str()); }
                }
            } else {
                if(fDebug) { LogPrintf("CheckBlock() : pindex is null, skipping floatingcity payment check\n"); }
            }
        } else {
            if(fDebug) { LogPrintf("CheckBlock() : skipping floatingcity payment checks\n"); }
        }
    } else {
        if(fDebug) { LogPrintf("CheckBlock() : Is initial download, skipping floatingcity payment check %d\n", pindexBest->nHeight+1); }
    }
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if(!IsInitialBlockDownload()){
        ...
        // If we're in LiteMode disable fcengine features without disabling floatingcities
        if (!fLiteMode && !fImporting && !fReindex && pindexBest->nHeight > Checkpoints::GetTotalBlocksEstimate()){

            if(floatingcityPayments.GetBlockPayee(pindexBest->nHeight, payee, vin)){
                //UPDATE FLOATINGCITY LAST PAID TIME
                CFloatingcity* pmn = mnodeman.Find(vin);
                if(pmn != NULL) {
                    pmn->nLastPaid = GetAdjustedTime();
                }

                LogPrintf("ProcessBlock() : Update Floatingcity Last Paid Time - %d\n", pindexBest->nHeight);
            }

            mnEnginePool.CheckTimeout();
            mnEnginePool.NewBlock();
            floatingcityPayments.ProcessBlock(GetHeight()+10);
...
{
    RenameThread("Zalem-Coin-loadblk");
...
bool static AlreadyHave(CTxDB& txdb, const CInv& inv)
{
    ...
    case MSG_FLOATINGCITY_WINNER:
        return mapSeenFloatingcityVotes.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_FLOATINGCITY_WINNER) {
                    if(mapSeenFloatingcityVotes.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mapSeenFloatingcityVotes[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "tx"|| strCommand == "dstx")
    {
        ...
        //floatingcity signed transaction
        ...
        if(strCommand == "tx") {
            ...
            if (AlreadyHave(txdb, inv))
                return true;
            //these allow floatingcities to publish a limited amount of free transactions

            CFloatingcity* pmn = mnodeman.Find(vin);
            if(pmn != NULL)
            {
                if(!pmn->allowFreeTx){
                    //multiple peers can send us a valid floatingcity transaction
                    if(fDebug) LogPrintf("dstx: Floatingcity sending too many transactions %s\n", tx.GetHash().ToString().c_str());
                    return true;
                }
                ...
                if(!mnEngineSigner.VerifyMessage(pmn->pubkey2, vchSig, strMessage, errorMessage)){
                    LogPrintf("dstx: Got bad floatingcity address signature %s \n", vin.ToString().c_str());
                    //Misbehaving(pfrom->GetId(), 20);
                    return false;
                }

                LogPrintf("dstx: Got Floatingcity transaction %s\n", tx.GetHash().ToString().c_str());
    ...
    else
    {
        ...
        ProcessMessageFloatingcityPayments(pfrom, strCommand, vRecv);
