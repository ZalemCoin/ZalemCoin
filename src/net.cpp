...
{
    // Look for an existing connection. If found then just add it to floatingcity list.
    ...
    {
        LogPrint("net", "connected floatingcity %s\n", addrConnect.ToString());
        closesocket(hSocket);
...
*/
    ...
    LogPrint("net", "connecting to floatingcity %s failed\n", addrConnect.ToString());
    return false;
}
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (ZalemCoin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Zalem-Coin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Zalem-Coin is probably already running."), addrBind.ToString());
        else
