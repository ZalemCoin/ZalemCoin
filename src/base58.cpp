...
namespace {
    class CZalemCoinAddressVisitor : public boost::static_visitor<bool> {
    private:
        CZalemCoinAddress *addr;
    public:
        CZalemCoinAddressVisitor(CZalemCoinAddress *addrIn) : addr(addrIn) { }
...
bool CZalemCoinAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CZalemCoinAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CZalemCoinAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CZalemCoinAddressVisitor(this), dest);
}

bool CZalemCoinAddress::IsValid() const {
...
CTxDestination CZalemCoinAddress::Get() const {
    if (!IsValid())
...
bool CZalemCoinAddress::GetKeyID(CKeyID &keyID) const {
...
bool CZalemCoinAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CZalemCoinSecret::SetKey(const CKey& vchSecret) {
...
CKey CZalemCoinSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CZalemCoinSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CZalemCoinSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CZalemCoinSecret::SetString(const std::string& strSecret) {
