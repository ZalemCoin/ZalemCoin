...
//#include "fcengine-relay.h"
#include "activefloatingcity.h"
#include "floatingcity-payments.h"
...
void Shutdown()
{
    ...
    RenameThread("Zalem-Coin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Zalem-Coin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Zalem-Coind.pid)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the ZalemCoin Wiki for SSL setup instructions)") + "\n";
    ...
    strUsage += "  -litemode=<n>          " + _("Disable all FloatingCity related functionality (0-1, default: 0)") + "\n";
    strUsage += "\n" + _("Floatingcity options:") + "\n";
    strUsage += "  -floatingcity=<n>            " + _("Enable the client to act as a floatingcity (0-1, default: 0)") + "\n";
    strUsage += "  -mnconf=<file>             " + _("Specify floatingcity configuration file (default: floatingcity.conf)") + "\n";
    strUsage += "  -mnconflock=<n>            " + _("Lock floatingcities from floatingcity configuration file (default: 1)") + "\n";
    strUsage += "  -floatingcityprivkey=<n>     " + _("Set the floatingcity private key") + "\n";
    strUsage += "  -floatingcityaddr=<n>        " + _("Set external address:port to get to this floatingcity (example: address:port)") + "\n";
    strUsage += "  -floatingcityminprotocol=<n> " + _("Ignore floatingcities less than version (example: 61401; default : 0)") + "\n";
...
/** Sanity checks
 *  Ensure that ZalemCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Process floatingcity config
    floatingcityConfig.read(GetFloatingcityConfigFile());
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Zalem-Coin is shutting down."));
...
#endif
    // Make sure only a single ZalemCoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Zalem-Coin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Zalem-Coin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (mapArgs.count("-floatingcitypaymentskey")) // floatingcity payments priv key
    {
        if (!floatingcityPayments.SetPrivKey(GetArg("-floatingcitypaymentskey", "")))
            return InitError(_("Unable to sign floatingcity payment winner, wrong key?"));
        if (!sporkManager.SetPrivKey(GetArg("-floatingcitypaymentskey", "")))
            return InitError(_("Unable to sign spork message, wrong key?"));
    }

    //ignore floatingcities below protocol version
    nFloatingcityMinProtocol = GetArg("-floatingcityminprotocol", MIN_POOL_PEER_PROTO_VERSION);

    if (fDaemon)
        fprintf(stdout, "Zalem-Coin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Zalem-Coin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Zalem-Coin to complete") << "\n";
    ...
    uiInterface.InitMessage(_("Loading floatingcity cache..."));

    CFloatingcityDB mndb;
    CFloatingcityDB::ReadResult readResult = mndb.Read(mnodeman);
    if (readResult == CFloatingcityDB::FileError)
        LogPrintf("Missing floatingcity cache file - mncache.dat, will try to recreate\n");
    ...
    fFloatingCity = GetBoolArg("-floatingcity", false);
    if(fFloatingCity) {
        LogPrintf("IS FLOATING CITY\n");
        strFloatingCityAddr = GetArg("-floatingcityaddr", "");

        LogPrintf(" addr %s\n", strFloatingCityAddr.c_str());

        if(!strFloatingCityAddr.empty()){
            CService addrTest = CService(strFloatingCityAddr, fNameLookup);
            if (!addrTest.IsValid()) {
                return InitError("Invalid -floatingcityaddr address: " + strFloatingCityAddr);
            }
        }

        strFloatingCityPrivKey = GetArg("-floatingcityprivkey", "");
        if(!strFloatingCityPrivKey.empty()){
            std::string errorMessage;

            CKey key;
            CPubKey pubkey;

            if(!mnEngineSigner.SetKey(strFloatingCityPrivKey, errorMessage, key, pubkey))
            {
                return InitError(_("Invalid floatingcityprivkey. Please see documenation."));
            }

            activeFloatingcity.pubKeyFloatingcity = pubkey;

        } else {
            return InitError(_("You must specify a floatingcityprivkey in the configuration. Please see documentation for help."));
        }
    ...
    if(GetBoolArg("-mnconflock", false)) {
        LogPrintf("Locking Floatingcities:\n");
        uint256 mnTxHash;
    ...
    //lite mode disables all Floatingcity related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if(fFloatingCity && fLiteMode){
        return InitError("You can not start a floatingcity in litemode");
    }
