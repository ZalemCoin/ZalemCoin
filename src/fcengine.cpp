...
// The main object for accessing fcengine
CMNenginePool mnEnginePool;
// A helper object for signing messages from Floatingcities
CMNengineSigner mnEngineSigner;
// The current fcengines in progress on the network
std::vector<CMNengineQueue> vecMNengineQueue;
// Keep track of the used Floatingcities
std::vector<CTxIn> vecFloatingcitiesUsed;
// keep track of the scanning errors I've seen
map<uint256, CMNengineBroadcastTx> mapMNengineBroadcastTxes;
// Keep track of the active Floatingcity
CActiveFloatingcity activeFloatingcity;

// count peers we've requested the list from
int RequestedFloatingCityList = 0;

/* *** BEGIN FLOATINGCITY MAGIC - DASH **********
...
void CMNenginePool::SetNull(){
    ...
    sessionFoundFloatingcity = false;
...
//
// Check the MNengine progress and send client updates if a Floatingcity
//
void CMNenginePool::Check()
{
    if(fFloatingCity) LogPrint("fcengine", "CMNenginePool::Check() - entries count %lu\n", entries.size());
    //printf("CMNenginePool::Check() %d - %d - %d\n", state, anonTx.CountEntries(), GetTimeMillis()-lastTimeChanged);

    if(fFloatingCity) {
        LogPrint("fcengine", "CMNenginePool::Check() - entries count %lu\n", entries.size());
        // If entries is full, then move on to the next phase
        if(state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions())
        {
            LogPrint("fcengine", "CMNenginePool::Check() -- TRYING TRANSACTION \n");
    ...
    if(state == POOL_STATUS_FINALIZE_TRANSACTION) {
        LogPrint("fcengine", "CMNenginePool::Check() -- FINALIZE TRANSACTIONS\n");
        ...
        if (fFloatingCity) {
            ...
            LogPrint("fcengine", "Transaction 1: %s\n", txNew.ToString());
    ...
    if(fFloatingCity && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        LogPrint("fcengine", "CMNenginePool::Check() -- SIGNING\n");
    ...
    if((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis()-lastTimeChanged >= 10000) {
        LogPrint("fcengine", "CMNenginePool::Check() -- timeout, RESETTING\n");
        UnlockCoins();
        SetNull();
        if(fFloatingCity) RelayStatus(sessionID, GetState(), GetEntriesCount(), FLOATINGCITY_RESET);
    }
}

void CMNenginePool::CheckFinalTransaction()
{
    if (!fFloatingCity) return; // check and relay final tx only on floatingcity

    CWalletTx txNew = CWalletTx(pwalletMain, finalTransaction);

    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        LogPrint("fcengine", "Transaction 2: %s\n", txNew.ToString());
        ...
        if(!mnEngineSigner.SetKey(strFloatingCityPrivKey, strError, key2, pubkey2))
        {
            LogPrintf("CMNenginePool::Check() - ERROR: Invalid Floatingcityprivkey: '%s'\n", strError);
            return;
        }
        ...
        if(!mapMNengineBroadcastTxes.count(txNew.GetHash())){
            ...
            dstx.tx = txNew;
            dstx.vin = activeFloatingcity.vin;
            dstx.vchSig = vchSig;
        ...
        // Reset
        LogPrint("fcengine", "CMNenginePool::Check() -- COMPLETED -- RESETTING \n");
        SetNull();
        RelayStatus(sessionID, GetState(), GetEntriesCount(), FLOATINGCITY_RESET);
    }
}
...
// The submission and signing stages in fcengine are completely separate. In the cases where
// a client submits a transaction then refused to sign, there must be a cost. Otherwise they
// would be able to do this over and over again and bring the mixing to a hault.
//
// How does this work? Messages to Floatingcities come in via "dsi", these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Floatingcity
    ...
    if(fFloatingCity) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Zalem-Coin and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002 ZLM per transaction on average.
            */
            if(r <= 10)
            {
...
//
// Check for various timeouts (queue objects, fcengine, etc)
//
void CMNenginePool::CheckTimeout(){
    ...
    if(!fFloatingCity) {
        switch(state) {
            case POOL_STATUS_TRANSMISSION:
                LogPrint("fcengine", "CMNenginePool::CheckTimeout() -- Session complete -- Running Check()\n");
                Check();
                break;
            case POOL_STATUS_ERROR:
                LogPrint("fcengine", "CMNenginePool::CheckTimeout() -- Pool error -- Running Check()\n");
                Check();
                break;
            case POOL_STATUS_SUCCESS:
                LogPrint("fcengine", "CMNenginePool::CheckTimeout() -- Pool success -- Running Check()\n");
                Check();
                break;
        }
    }
    ...
    while(it != vecMNengineQueue.end()){
        if((*it).IsExpired()){
            LogPrint("fcengine", "CMNenginePool::CheckTimeout() : Removing expired queue entry - %d\n", c);
    ...
    int addLagTime = 0;
    if(!fFloatingCity) addLagTime = 10000; //if we're the client, give the server a few extra seconds before resetting.
    ...
    if(state == POOL_STATUS_ACCEPTING_ENTRIES || state == POOL_STATUS_QUEUE){
        ...
        while(it2 != entries.end()){
            if((*it2).IsExpired()){
                LogPrint("fcengine", "CMNenginePool::CheckTimeout() : Removing expired entry - %d\n", c);
        ...
        LogPrint("fcengine", "CMNenginePool::CheckTimeout() -- Session timed out (%ds) -- resetting\n", MNengine_QUEUE_TIMEOUT);
        UnlockCoins();
        SetNull();
    ...
    if(state == POOL_STATUS_SIGNING && GetTimeMillis()-lastTimeChanged >= (MNengine_SIGNING_TIMEOUT*1000)+addLagTime ) {
            LogPrint("fcengine", "CMNenginePool::CheckTimeout() -- Session timed out (%ds) -- restting\n", MNengine_SIGNING_TIMEOUT);
            ChargeFees();
            UnlockCoins();
            SetNull();
...
void CMNenginePool::CheckForCompleteQueue(){
    ...
    if(state == POOL_STATUS_QUEUE && sessionUsers == GetMaxPoolTransactions()) {
        ...
        CMNengineQueue dsq;
        dsq.vin = activeFloatingcity.vin;
        dsq.time = GetTime();
        dsq.ready = true;
        dsq.Sign();
        dsq.Relay();
...
bool CMNenginePool::SignatureValid(const CScript& newSig, const CTxIn& newVin){
    ...
    if(found >= 0){ //might have to do this one input at a time?
        ...
        LogPrint("fcengine", "CMNenginePool::SignatureValid() - Sign with sig %s\n", newSig.ToString().substr(0,24));
        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, txNew, n, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, 0)){
            LogPrint("fcengine", "CMNenginePool::SignatureValid() - Signing - Error signing input %u\n", n);
            return false;
        }
    }

    LogPrint("fcengine", "CMNenginePool::SignatureValid() - Signing - Successfully validated input\n");
    return true;
}
...
bool CMNenginePool::IsCollateralValid(const CTransaction& txCollateral){
    ...
    if(missingTx){
        LogPrint("fcengine", "CMNenginePool::IsCollateralValid - Unknown inputs in collateral transaction - %s\n", txCollateral.ToString());
        return false;
    }
    ...
    if(nValueIn-nValueOut < MNengine_COLLATERAL) {
        LogPrint("fcengine", "CMNenginePool::IsCollateralValid - did not include enough fees in transaction %d\n%s\n", nValueOut-nValueIn, txCollateral.ToString());
        return false;
    }

    LogPrint("fcengine", "CMNenginePool::IsCollateralValid %s\n", txCollateral.ToString());
...
bool CMNenginePool::AddEntry(const std::vector<CTxIn>& newInput, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& newOutput, std::string& error){
    ...
    BOOST_FOREACH(CTxIn in, newInput) {
        if (in.prevout.IsNull() || nAmount < 0) {
            LogPrint("fcengine", "CMNenginePool::AddEntry - input not valid!\n");
    ...
    if (!IsCollateralValid(txCollateral)){
        LogPrint("fcengine", "CMNenginePool::AddEntry - collateral not valid!\n");
    ...
    if((int)entries.size() >= GetMaxPoolTransactions()){
        LogPrint("fcengine", "CMNenginePool::AddEntry - entries is full!\n");
    ...
    BOOST_FOREACH(CTxIn in, newInput) {
        LogPrint("fcengine", "looking for vin -- %s\n", in.ToString());
        BOOST_FOREACH(const CMNengineEntry& v, entries) {
            BOOST_FOREACH(const CTxDSIn& s, v.sev){
                if((CTxIn)s == in) {
                    LogPrint("fcengine", "CMNenginePool::AddEntry - found in vin\n");
    ...
    LogPrint("fcengine", "CMNenginePool::AddEntry -- adding %s\n", newInput[0].ToString());
    error = "";
...
bool CMNenginePool::AddScriptSig(const CTxIn& newVin){
    LogPrint("fcengine", "CMNenginePool::AddScriptSig -- new sig  %s\n", newVin.scriptSig.ToString().substr(0,24));

    BOOST_FOREACH(const CMNengineEntry& v, entries) {
        BOOST_FOREACH(const CTxDSIn& s, v.sev){
            if(s.scriptSig == newVin.scriptSig) {
                LogPrint("fcengine", "CMNenginePool::AddScriptSig - already exists \n");
                return false;
            }
        }
    }
    ...
    if(!SignatureValid(newVin.scriptSig, newVin)){
        LogPrint("fcengine", "CMNenginePool::AddScriptSig - Invalid Sig\n");
        return false;
    }

    LogPrint("fcengine", "CMNenginePool::AddScriptSig -- sig %s\n", newVin.ToString());

    BOOST_FOREACH(CTxIn& vin, finalTransaction.vin){
        if(newVin.prevout == vin.prevout && vin.nSequence == newVin.nSequence){
            vin.scriptSig = newVin.scriptSig;
            vin.prevPubKey = newVin.prevPubKey;
            LogPrint("fcengine", "CMNenginePool::AddScriptSig -- adding to finalTransaction  %s\n", newVin.scriptSig.ToString().substr(0,24));
        }
    }
    for(unsigned int i = 0; i < entries.size(); i++){
        if(entries[i].AddSig(newVin)){
            LogPrint("fcengine", "CMNenginePool::AddScriptSig -- adding  %s\n", newVin.scriptSig.ToString().substr(0,24));
            return true;
        }
    }
...
// Incoming message from Floatingcity updating the progress of fcengine
...
bool CMNenginePool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, std::string& error, int newSessionID){
    ...
    if(newState == POOL_STATUS_ACCEPTING_ENTRIES){
        if(newAccepted == 1){
            ...
            sessionFoundFloatingcity = true;
            //wait for other users. Floatingcity will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundFloatingcity) {
            LogPrintf("CMNenginePool::StatusUpdate - entry not accepted by Floatingcity \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
        }
        if(sessionFoundFloatingcity) return true;
    }
...
//
// After we receive the finalized transaction from the Floatingcity, we must
...
{
    LogPrint("fcengine", "CMNenginePool::NewBlock \n");
...
std::string CMNenginePool::GetMessageByID(int messageID) {
    ...
    case ERR_MN_LIST: return _("Not in the Floatingcity list.");
    case ERR_MODE: return _("Incompatible mode.");
    case ERR_NON_STANDARD_PUBKEY: return _("Non-standard public key detected.");
    case ERR_NOT_A_MN: return _("This is not a Floatingcity.");
    case ERR_QUEUE_FULL: return _("Floatingcity queue is full.");
...
bool CMNengineSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey){
    ...
    if(GetTransaction(vin.prevout.hash, txVin, hash)){
        BOOST_FOREACH(CTxOut out, txVin.vout){
            if(out.nValue == FloatingcityCollateral(pindexBest->nHeight)*COIN){
...
bool CMNengineQueue::Sign()
{
    ...
    if(!mnEngineSigner.SetKey(strFloatingCityPrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CMNengineQueue():Relay - ERROR: Invalid Floatingcityprivkey: '%s'\n", errorMessage);
        return false;
    }
...
bool CMNengineQueue::CheckSignature()
{
    ...
    if(pmn != NULL)
    {
        ...
        if(!mnEngineSigner.VerifyMessage(pmn->pubkey2, vchSig, strMessage, errorMessage)){
            return error("CMNengineQueue::CheckSignature() - Got bad Floatingcity address signature %s \n", vin.ToString().c_str());
        }
...
{
    if(!pSubmittedToFloatingcity) return;
    ...
    CNode* pnode = FindNode(pSubmittedToFloatingcity->addr);
    if(pnode != NULL) {
...
{
    if(fLiteMode) return; //disable all MNengine/Floatingcity related functionality

    // Make this thread recognisable as the wallet flushing thread
    RenameThread("Zalem-Coin-fcengine");
    ...
    while (true)
    {
        ...
        if(mnEnginePool.IsBlockchainSynced()) {
            ...
            if(c % FLOATINGCITY_PING_SECONDS == 1) activeFloatingcity.ManageStatus();

            if(c % 60 == 0)
            {
                mnodeman.CheckAndRemove();
                mnodeman.ProcessFloatingcityConnections();
                floatingcityPayments.CleanPaymentList();
                CleanTransactionLocksList();
            }

            //if(c % FLOATINGCITYS_DUMP_SECONDS == 0) DumpFloatingcities();
