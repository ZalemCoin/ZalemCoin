...
CCriticalSection cs_floatingcities;
// keep track of the scanning errors I've seen
map<uint256, int> mapSeenFloatingcityScanningErrors;
...
CFloatingcity::CFloatingcity()
{
    ...
    activeState = FLOATINGCITY_ENABLED;
...
CFloatingcity::CFloatingcity(const CFloatingcity& other)
{
    LOCK(cs);
    vin = other.vin;
    addr = other.addr;
...
CFloatingcity::CFloatingcity(CService newAddr, CTxIn newVin, CPubKey newPubkey, std::vector<unsigned char> newSig, int64_t newSigTime, CPubKey newPubkey2, int protocolVersionIn, CScript newDonationAddress, int newDonationPercentage)
{
    LOCK(cs);
    vin = newVin;
    addr = newAddr;
...
//
// Deterministically calculate a given "score" for a floatingcity depending on how close it's hash is to
// the proof of work for that block. The further away they are the better, the furthest will win the election
// and get paid this block
//
uint256 CFloatingcity::CalculateScore(int mod, int64_t nBlockHeight)
{
...
void CFloatingcity::Check()
{
    ...
    if(activeState == FLOATINGCITY_VIN_SPENT) return;


    if(!UpdatedWithin(FLOATINGCITY_REMOVAL_SECONDS)){
        activeState = FLOATINGCITY_REMOVE;
        return;
    }

    if(!UpdatedWithin(FLOATINGCITY_EXPIRATION_SECONDS)){
        activeState = FLOATINGCITY_EXPIRED;
        return;
    }
	...
	if(!AcceptableInputs(mempool, tx, false, NULL)){
            activeState = FLOATINGCITY_VIN_SPENT;
            return;
        }
    }

    activeState = FLOATINGCITY_ENABLED; // OK
}
