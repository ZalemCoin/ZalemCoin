...
public:
    ...
    bool SelectCoinsFloatingcity(CTxIn& vin, int64_t& nValueRet, CScript& pubScript) const;
    ...
    bool ImportPrivateKey(CZalemCoinSecret vchSecret, string strLabel = "", bool fRescan = true);
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache=true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            ...
            if(fFloatingCity && vout[i].nValue == FloatingcityCollateral(pindexBest->nHeight)*COIN) continue; // do not count MN-like outputs
