...
{
    QString ret = QString("Zalem-Coin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ZLM unit input in lnReqAmount, we generate the URI with ZLM as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(ZalemCoinUnits::format(ZalemCoinUnits::ZLM, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
