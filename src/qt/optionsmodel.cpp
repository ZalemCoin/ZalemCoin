...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", ZalemCoinUnits::ZLM);
    ...
    if (!settings.contains("nAnonymizeZalemCoinAmount"))
        settings.setValue("nAnonymizeZalemCoinAmount", 1000);
    nAnonymizeZalemCoinAmount = settings.value("nAnonymizeZalemCoinAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-fcenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeZalemCoinAmount"))
        SoftSetArg("-anonymizeZalemCoinamount", settings.value("nAnonymizeZalemCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeZalemCoinAmount:
            return QVariant(nAnonymizeZalemCoinAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            emit fcengineRoundsChanged(nMNengineRounds);
            break;
        case AnonymizeZalemCoinAmount:
            nAnonymizeZalemCoinAmount = value.toInt();
            settings.setValue("nAnonymizeZalemCoinAmount", nAnonymizeZalemCoinAmount);
            emit AnonymizeZalemCoinAmountChanged(nAnonymizeZalemCoinAmount);
            break;
        default:
            break;
        }
