...
class CFloatingCityVote;
class CZalemCoinAddress;
...
#define POOL_STATUS_FINALIZE_TRANSACTION       4 // floating city will broadcast what it accepted
...
#define FLOATINGCITY_ACCEPTED                    1
#define FLOATINGCITY_REJECTED                    0
#define FLOATINGCITY_RESET                       -1
...
extern std::string strFloatingCityPrivKey;
extern map<uint256, CMNengineBroadcastTx> mapMNengineBroadcastTxes;
extern CActiveFloatingcity activeFloatingcity;
...
// A clients transaction in the fcengine pool
...
public:
     ...
     *     1) we have an active Floatingcity,
     *     2) we have a valid Floatingcity private key,
    ...
    /// Check if we have a valid Floatingcity address
    bool CheckSignature();
...
{
public:
    /// Is the inputs associated with this public key? (and there is 10000 ZLM - checking if valid floatingcity)
...
{
private:
    mutable CCriticalSection cs_fcengine;

    std::vector<CMNengineEntry> entries; // Floatingcity entries
    ...
    bool sessionFoundFloatingcity; //If we've found a compatible Floatingcity
...
public:
    ...
    CFloatingcity* pSubmittedToFloatingcity;
    ...
    {
        if (fFloatingCity && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)){
            LogPrint("fcengine", "CMNenginePool::UpdateState() - Can't set state to ERROR or SUCCESS as a Floatingcity. \n");
            return;
        }
        ...
        if(state != newState){
            ...
            if(fFloatingCity) {
                RelayStatus(mnEnginePool.sessionID, mnEnginePool.GetState(), mnEnginePool.GetEntriesCount(), FLOATINGCITY_RESET);
            }
        }
        state = newState;
    }
    ...
    /// Get Floatingcity updates about the progress of MNengine
