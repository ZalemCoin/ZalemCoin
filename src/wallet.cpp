...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CZalemCoinAddress(redeemScript.GetID()).ToString();
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s ZLM %s\n", FormatMoney(wtx.GetCredit(ISMINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_NOT10000IFMN) {
                    found = !(fFloatingCity && pcoin->vout[i].nValue == FloatingcityCollateral(pindexBest->nHeight)*COIN);
                } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN){
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    if(fFloatingCity) found = pcoin->vout[i].nValue != FloatingcityCollateral(pindexBest->nHeight)*COIN; // do not use Hot MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
...
void CWallet::AvailableCoinsForStaking(vector<COutput>& vCoins, unsigned int nSpendTime) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            bool found = false;
            for (unsigned int i = 0; i < pcoin->vout.size(); i++){
                if (pcoin->vout[i].nValue == FloatingcityCollateral(pindexBest->nHeight)*COIN){

                    //LogPrintf("CWallet::AvailableCoinsForStaking - Found Floatingcity collateral.\n");
                    found = true;
                    break;
                }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(" Unable to locate enough MNengine non-denominated funds for this transaction that are not equal 1000 ZLM.");
                    }
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-Zalem-Coin-address
                    CScript scriptChange;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        if (fDebug)
        {
            ...
            CZalemCoinAddress coinAddress(keyID);
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    CZalemCoinAddress addrTo(ckidTo);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Floatingcity Payments
    int payments = 1;
    // start floatingcity payments
    bool bFloatingCityPayment = false;

    if ( Params().NetworkID() == CChainParams::TESTNET ){
        if (GetTime() > START_FLOATINGCITY_PAYMENTS_TESTNET ){
            bFloatingCityPayment = true;
        }
    }else{
        if (GetTime() > START_FLOATINGCITY_PAYMENTS){
            bFloatingCityPayment = true;
        }
    }
    // stop floatingcity payments (for testing)
    if ( Params().NetworkID() == CChainParams::TESTNET ){
        if (GetTime() > STOP_FLOATINGCITY_PAYMENTS_TESTNET ){
            bFloatingCityPayment = false;
        }
    }else{
        if (GetTime() > STOP_FLOATINGCITY_PAYMENTS){
            bFloatingCityPayment = false;
        }
    }

    bool hasPayment = true;
    if(bFloatingCityPayment) {
        //spork
        if(!floatingcityPayments.GetBlockPayee(pindexPrev->nHeight+1, payee, vin)){
            CFloatingcity* winningNode = mnodeman.GetCurrentFloatingCity(1);
            if(winningNode){
                payee = GetScriptForDestination(winningNode->pubkey.GetID());
            } else {
                return error("CreateCoinStake: Failed to detect floatingcity to pay\n");
            }
        }
    } else {
        hasPayment = false;
    ...
    if(hasPayment){
        ...
        LogPrintf("Floatingcity payment to %s\n", address2.ToString().c_str());
    }
    ...
    if(bDevOpsPayment) {
        ...
        if (Params().NetworkID() == CChainParams::MAIN)
            devopaddress = CBitcoinAddress("ZXUiExq15ATbkeEBB2FUXFqpJQGKk2iGAd"); // TODO: nothing, already set to a valid Zalem address
    ...
    int64_t floatingcityPayment = GetFloatingcityPayment(pindexPrev->nHeight+1, nReward);
    int64_t devopsPayment = GetDevOpsPayment(pindexPrev->nHeight+1, nReward); // TODO: Activate devops


    // Set output amount
    // Standard stake (no Floatingcity or DevOps payments)
    if (!hasPayment && !hasdevopsPay)
    {
        if(txNew.vout.size() == 3){ // 2 stake outputs, stake was split, no floatingcity payment
            txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
            txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
        }
        else if(txNew.vout.size() == 2){ // only 1 stake output, was not split, no floatingcity payment
            txNew.vout[1].nValue = blockValue;
        }
    }
    else if(hasPayment && !hasdevopsPay)
    {
        if(txNew.vout.size() == 4){ // 2 stake outputs, stake was split, plus a floatingcity payment
            txNew.vout[payments-1].nValue = floatingcityPayment;
            blockValue -= floatingcityPayment;
            txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
            txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
        }
        else if(txNew.vout.size() == 3){ // only 1 stake output, was not split, plus a floatingcity payment
            txNew.vout[payments-1].nValue = floatingcityPayment;
            blockValue -= floatingcityPayment;
            txNew.vout[1].nValue = blockValue;
        }
    ...
    {
        if(txNew.vout.size() == 5){ // 2 stake outputs, stake was split, plus a devops AND floatingcity payment
            txNew.vout[payments-1].nValue = floatingcityPayment;
            blockValue -= floatingcityPayment;
            txNew.vout[devoppay-1].nValue = devopsPayment;
            blockValue -= devopsPayment;
        ...
        else if(txNew.vout.size() == 4){ // only 1 stake output, was not split, plus a devops AND floatingcity payment
            txNew.vout[payments-1].nValue = floatingcityPayment;
            blockValue -= floatingcityPayment;
            txNew.vout[devoppay-1].nValue = devopsPayment;
            blockValue -= devopsPayment;
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CZalemCoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CZalemCoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CZalemCoinAddress(address).ToString());
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s ZLM %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s ZLM %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool CWallet::ImportPrivateKey(CZalemCoinSecret vchSecret, string strLabel, bool fRescan) {
    if (fWalletUnlockStakingOnly)
        return false;
